...
本仓库负责把外部全流程规划结果转换为机器人语义动作：准备区读取 `param_config` 参数，竞技区暂时读取 `match_config` 固定参数，梅林区通过 `/get_action_seq` 拉取外部暂存的 `/mf_action_seq`，由固定 BehaviorTree.CPP 顶层树和 3 个子树调度底盘、悬挂、机械臂和 ARES tool service。本仓库不负责路径规划、KFS 选择、入口/出口选择或路线合法性判断。
...
| `meilin_default_align_timeout` | `30.0` | MoveToPose 超时 (s) |
...
`PrepareArea` 从 `r2_bt/config/param.yaml` 的 `prepare_area` 读取固定参数：
...
```bash
ros2 launch r2_bt bt_engine.launch.py param_config:=config/param.yaml
```
