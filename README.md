...
本仓库负责把外部全流程规划结果转换为机器人语义动作：准备区和竞技区读取 `match_config` 固定参数，梅林区通过 `/get_action_seq` 拉取外部暂存的 `/mf_action_seq`，由固定 BehaviorTree.CPP 顶层树和 3 个子树调度底盘、悬挂、机械臂和 ARES tool service。本仓库不负责路径规划、KFS 选择、入口/出口选择或路线合法性判断。
...
r2_bt (bt_engine_node)
  ...
  │     ├── PrepareArea  (MoveToPose 到矛头准备区，等待 ARES tool service)
...
| 子树 | 覆盖流程 | 调度方式 |
|------|----------|----------|
| `PrepareArea` | `MoveToPose` 到矛头准备区，然后等待 `/ares_tool_node/tool_action` 返回成功 | 固定参数（match_config），直接读 blackboard |
...
```text
PREPARE_01_MOVE_TO_SPEAR_AREA
  MoveToPose(target = prepare.spear_prep)

PREPARE_02_WAIT_ARES_TOOL
  AresToolAction(service = /ares_tool_node/tool_action, action = prepare.ares_tool.action)
```

PrepareArea 不再执行旧的 `SpearAction prep/grasp/dock_extend/dock_release` 链路，也不负责对接后的梅林入口移动。`AresToolAction` 收到 `success=true && ret=0` 后，PrepareArea 返回 SUCCESS，顶层树自动进入 `MeilinArea`；梅林入口运动由 `/mf_action_seq` 的第一段 `move` 负责。
...
`PrepareArea` 从 `r2_bt/config/match_*.json` 读取固定参数：
