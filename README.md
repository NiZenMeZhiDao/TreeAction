...
本仓库负责把外部全流程规划结果转换为机器人语义动作：准备区、梅林区和竞技区固定参数读取 `param_config`（推荐 `param_blue.yaml` / `param_red.yaml`），梅林区通过 `/get_action_seq` 拉取外部暂存的 `/mf_action_seq`，由固定 BehaviorTree.CPP 顶层树和 3 个子树调度底盘、悬挂、机械臂和 ARES tool service。本仓库不负责路径规划、KFS 选择、入口/出口选择或路线合法性判断。

## 架构

```text
外部规划模块 + ARES tool service
  ├── /get_action_seq -> /mf_action_seq (std_msgs/Float32MultiArray)  ← 梅林区路径
  └── /ares_tool_node/tool_action (r2_interfaces/srv/ToolAction)      ← 准备区 tool 动作
        ↓
r2_bt (bt_engine_node)
  ├── 固定 BT XML: full_match.xml
  │     ├── PrepareArea  (抓取矛头，并行移动到对接待定区与 dock_extend)
  ...
  └── Service Clients
        ↓
r2_hardware / ares_tool_control
  ├── move_to_pose                (底盘导航/精调)
  ├── suspension_control          (四轮独立悬挂)
  ├── arm_action                  (机械臂语义控制)
  ├── spear_action                (矛头机构控制，保留接口)
  └── tool_action                 (ARES tool USB service)
        ↓
...
```text
mf_action_planner / mf_buffer_node (外部)
      │  /get_action_seq -> /mf_action_seq  std_msgs/Float32MultiArray
      ▼
r2_bt::bt_engine_node
...
onStart():
  ...
  6. pre_align 判断（如需回中，与 climb 并行发出）:
...
Phase 流程（onStart 直接发 action，tick 检查并推进）:
  Phase::START — 并行发出 pre_align + climb:
    ├── if pre_align_needed  → MoveToPose(pre_align)
    └── if climb_needed      → SuspensionControl(climb)
         → Phase::WAIT_PHASE1

  Phase::WAIT_PHASE1 — 等 pre_align 完成（climb 继续并行）:
    └── pre_align done?  → 发 MoveToPose(target)（与 climb 并行）
         → Phase::WAIT_PHASE2
  ...
  执行时序示意（有 pre_align + climb 的完整路径）:
    tick0:  ┬ MoveToPose(pre_align)
            └ Suspension(climb)
    tick1:  ┬ MoveToPose(pre_align) ✓ → 发 MoveToPose(target)
            └ Suspension(climb) 继续
    tick2:  ┬ MoveToPose(target) ✓
            └ Suspension(climb) ✓ → DONE
```

> **当前状态**: 完整计算 + RCLCPP_INFO 日志输出。MoveToPose 串行（pre_align → target），悬挂与 MoveToPose 并行。代码见 `r2_bt/src/nodes/meilin_move.cpp`。

### Fetch 详细逻辑

和 Move 类似：**MoveToPose 和 ArmAction 串行，Suspension 与 MoveToPose 并行**。
...
Phase 流程（onStart 直接发 action，tick 检查并推进）:
  Phase::START — 并行发出 MoveToPose(grasp) + Suspension:
    ├── MoveToPose(grasp)              ← 串行链起点
    └── if suspension_mode≠0 → SuspensionControl          ← 与 MoveToPose 并行
         → Phase::WAIT_PHASE1

  Phase::WAIT_PHASE1 — 等 MoveToPose 和 Suspension 都完成:
    └── both done?  → Phase::START_ARM

  Phase::START_ARM — MoveToPose 完成后才发 Arm（不能边动边抓）:
...
> **当前状态**: 完整计算 + RCLCPP_INFO 日志输出，真实调用 MoveToPose / SuspensionControl / ArmAction。代码见 `r2_bt/src/nodes/meilin_fetch.cpp`。
...
`bt_engine_node` 启动时从 `param_config.meilin_area` 加载，写入 blackboard 的 `meilin_config` (MeilinConfig 结构体)。红蓝方由 `param_config.match.side` 控制：

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `match.side` | `blue` | `blue`/`red`，影响高度地图 (row=2 红蓝相反) |
| `meilin_area.grid_size` | `1.2` | 格子尺寸 (m) |
| `meilin_area.grid_origin` | `[2.14, 0.49]` | (0,0) 格子中心世界坐标 |
| `meilin_area.grasp_distance` | `0.4` | 抓取时车身距格子边线距离 (m) |
| `meilin_area.yaw_tolerance` | `0.12` | yaw 校验容差 (rad) |
| `meilin_area.height_tolerance` | `1.0` | 高度差判断阈值 (mm) |
| `meilin_area.initial_grid` | `[0, 0]` | 初始位置 grid index |
| `meilin_area.initial_height` | `0.0` | 初始平台高度 (mm) |
| `meilin_area.initial_yaw` | `0.0` | 初始朝向 (rad) |
| `meilin_area.default_align_timeout` | `30.0` | MoveToPose 超时 (s) |
| `meilin_area.default_suspension_timeout` | `10.0` | SuspensionControl 超时 (s) |
| `meilin_area.default_grasp_timeout` | `30.0` | ArmAction 超时 (s) |
...
# 全场仿真（准备区→梅林区→竞技区）
ros2 launch r2_bringup r2_sim.launch.py param_config:=config/param_blue.yaml

# 全场实车（蓝方 / 红方）
ros2 launch r2_bringup r2_autonomy.launch.py startup_profile:=full param_config:=config/param_blue.yaml
ros2 launch r2_bringup r2_autonomy.launch.py startup_profile:=full param_config:=config/param_red.yaml
...
BT 引擎当前主要使用固定树和 service/Action 调度：

```text
/get_action_seq           r2_interfaces/srv/GetActionSeq   ← 拉取梅林区 /mf_action_seq 暂存数据
/transformed/pose         geometry_msgs/PoseStamped         ← map 系机器人中心/base_link 定位
/ares_tool_node/tool_action r2_interfaces/srv/ToolAction    ← 准备区 ARES tool 动作
```

`/planning/segments` JSON 路径仍保留兼容解析代码，但 `full_match.xml` 的准备区/竞技区已改为固定参数，不再依赖 JSON segment 队列。

梅林区使用：`PopNextMeilinSegment` → `SwitchSegmentType` → `Move` / `Fetch`（遇 `PLAN_DONE` 退出）。
...
| 子树 | 覆盖流程 | 调度方式 |
|------|----------|----------|
| `PrepareArea` | 移动到矛头抓取点并抓取，然后并行移动到对接待定区与 `dock_extend` | 固定参数（param_config），直接读 blackboard |
| `MeilinArea` | 梅林区格间移动、上下台阶、抓取 KFS | `ForEachSegment` → `PopNextMeilinSegment` → `Move`/`Fetch` |
| `FinalArea` | 竞技区中层放置 KFS、与 R1 合体放上层、安全收尾 | 固定参数（param_config，match_config 仅旧兼容），直接读 blackboard |

当前 PrepareArea 实际 XML 形态：

```text
PREPARE_01_MOVE_TO_SPEAR_PICKUP
  MoveToPose(target = prepare_area.points.spear_pickup)

PREPARE_02_GRASP_SPEAR
  AresToolAction(service = /ares_tool_node/tool_action, action = grasp)

PREPARE_03_DOCK_STANDBY_AND_EXTEND
  Parallel(success_count=2, failure_count=1)
    MoveToPose(target = prepare_area.points.dock_standby)
    AresToolAction(service = /ares_tool_node/tool_action, action = dock_extend, timeout = 60s)
```

PrepareArea 不再执行旧的 `SpearAction prep/grasp/dock_extend/dock_release` 链路。并行阶段中 `dock_extend` service 返回成功且底盘到达 `dock_standby` 后，PrepareArea 返回 SUCCESS，顶层树自动进入 `MeilinArea`；梅林入口运动由 `/mf_action_seq` 的第一段 `move` 负责。
...
| `Move` | `StatefulActionNode` | 梅林区格间移动：grid→world、爬台、pre_align、调用 MoveToPose/SuspensionControl |
| `Fetch` | `StatefulActionNode` | 梅林区抓取：KFS→抓取位姿、悬挂调整、调用 MoveToPose/SuspensionControl/ArmAction |
| `AresToolAction` | `StatefulActionNode` | 调用 `/ares_tool_node/tool_action` service，等待 ARES tool 动作完成 |
...
| `SpearAction` | `StatefulActionNode` | 异步调用 `spear_action`（保留接口，当前 PrepareArea 主流程不使用） |
...
| `SpearAction` | `spear_action` | 矛头机构控制（保留接口） |
| `ToolAction` | `/ares_tool_node/tool_action` | ARES tool service：`prepare` / `grasp` / `dock_extend` / arm tool 动作；定义在 `r2_interfaces/srv/ToolAction.srv` |

ROS 2 Action 通过 `rclcpp_action` / `rclpy.action` 异步调用；`ToolAction` 是 ROS 2 service，BT 节点 `AresToolAction` 发送请求后等待 service response，`success=true && ret=0` 才返回 SUCCESS。

`ares_tool_interfaces` 独立接口包已移除；ARES tool service 统一使用 `r2_interfaces/srv/ToolAction`。

### Action 实现状态

| 接口 | 定义 | BT 节点 (C++) | Server | 硬件通道 |
|------|:--:|:--:|:--:|------|
| `MoveToPose` | ✅ | ✅ | ✅ | `/t0x0101` |
| `SuspensionControl` | ✅ | ✅ | ✅ | `/t0x0102_action` |
| `ArmAction` | ✅ | ✅ | ✅ | `/t0x0103_` |
| `SpearAction` | ✅ | ✅ | ✅ | `/t0x0104_` |
| `ToolAction` | ✅ (`r2_interfaces/srv/ToolAction.srv`) | ✅ (`AresToolAction`) | ✅ (`ares_tool_control/tool_node`) | ARES USB sync frame |

## 配置示例（全区域 param_config）

正式比赛推荐显式选择红蓝配置；`config/param.yaml` 只是默认调试文件：
...
`param_config` 包含 `match` / `prepare_area` / `meilin_area` / `final_area`。完整文件见 `r2_bt/config/param_blue.yaml` 与 `r2_bt/config/param_red.yaml`：
...
meilin_area:
  ...
  suspension_normal_height: 30.0

final_area:
  deck_topic: "/aruco_comm/tx_id"
  place_signal: 7
...
`AresToolAction` 调用的 service 类型是 `r2_interfaces/srv/ToolAction`：

```bash
ros2 service call /ares_tool_node/tool_action r2_interfaces/srv/ToolAction "{action: 'grasp'}"
```
...
```json
{
  ...
  "segments": [
    ...
    {"segment_type": "MOVE2", "move_target": {"x": 0.8, "y": 0.0, "yaw": 0.0}},
    {
      "segment_type": "MOVE2",
...
```bash
colcon build --symlink-install --packages-select r2_interfaces ares_tool_control r2_bt r2_hardware r2_bringup
source install/setup.bash
```

`ToolAction.srv` 已合并在 `r2_interfaces` 内，因此不需要单独构建 `ares_tool_interfaces`。
...
ros2 launch r2_bringup r2_autonomy.launch.py startup_profile:=full param_config:=config/param_red.yaml tool_vid:=4617 tool_pid:=2 tool_completion_timeout_ms:=60000

# 梅林区实车（只启动梅林段依赖，等待 service 触发 BT）
ros2 launch r2_bringup r2_autonomy.launch.py startup_profile:=minimal_meilin param_config:=config/param_blue.yaml

# 启动梅林段执行
ros2 service call /bt_engine/start_autonomy r2_interfaces/srv/StartAutonomy "{region: 'meilin'}"
```

`r2_autonomy.launch.py` 会启动实车依赖节点并默认 `autostart:=false`，BT 节点会等待 `/bt_engine/start_autonomy` 后才开始 tick。`/mf_action_seq` 和 `/get_action_seq` 只提供梅林路径数据，不触发执行。`match_config` JSON 仍保留兼容旧流程；新流程推荐只通过 `param_config:=config/param_blue.yaml` 或 `param_config:=config/param_red.yaml` 选择红蓝区。仿真 launch 默认不启动 USB tool service；如需在仿真里跑完整 `PrepareArea`，需要另外提供同名 service 或单独启动可用的 tool 节点。
